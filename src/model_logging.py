...
class TensorboardLogger(Logger):
    ...
    def audio_summary(self, tag, sample, step, sr=16000):
        ...
        with self.writer.as_default():
            tf.summary.audio(tag, tf.expand_dims(sample, axis=0), sample_rate=sr, max_outputs=4, step=step)
            self.writer.flush()
    ...
    def histo_summary(self, tag, values, step, bins=200):
        ...
        # Create and write Summary
        # todo commented out but needs fixing
        # with self.writer.as_default():
    ...
    def tensor_summary(self, tag, tensor, step):
        ...
        with self.writer.as_default():
            tf.summary.scalar(tag, tf_tensor, step)
            self.writer.flush()
